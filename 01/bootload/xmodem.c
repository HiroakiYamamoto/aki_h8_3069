...
/* Send transmission requirement until receipt is enabled. */
...
long xmodem_recv(char *buf)
{
	...
	while (1) {
		if (!receiving)
			xmodem_wait(); /* Send transmission requirement until receipt is enabled. */

		c = serial_recv_byte(SERIAL_DEFAULT_DEVICE);

		if (c == XMODEM_EOT) { /* end of receipt */
			serial_send_byte(SERIAL_DEFAULT_DEVICE, XMODEM_ACK);
			break;
		} else if (c == XMODEM_CAN) { /* cancel of receipt */
			return -1;
		} else if (c == XMODEM_SOH) { /* start of receipt */
			receiving++;
			r = xmodem_read_block(block_number, buf); /* receipt in block unit */
			if (r < 0) { /* receipt error */
				serial_send_byte(SERIAL_DEFAULT_DEVICE, XMODEM_NAK);
			} else { /* normal receipt */
				block_number++;
				size += r;
